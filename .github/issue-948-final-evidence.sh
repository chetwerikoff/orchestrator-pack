...
const legacyComplete=(claim,outcome)=>{const f=path.join(tmpdir(),`claim-${process.pid}-${Math.random()}.json`);writeFileSync(f,JSON.stringify(claim));try{return parseLastJson(run(input.pwshPath,['-NoProfile','-Command',`$ErrorActionPreference='Stop';$env:AO_REVIEW_START_MONOTONIC_NOW_MS='1000';. '${psQuote(legacyScript)}';$r=Get-Content -LiteralPath '${psQuote(f)}' -Raw|ConvertFrom-Json -AsHashtable;$o=Complete-ReviewStartClaim -ClaimResult $r -Outcome '${outcome}' -ReviewRuns @();$o|ConvertTo-Json -Compress -Depth 30`]).stdout)}finally{rmSync(f,{force:true})}};
...
async function concurrentAcquisition(){
  const ns=base('acq');mkdirSync(ns,{recursive:true});const start=path.join(ns,'start');const release=path.join(ns,'release');const lr=path.join(ns,'legacy.json');const cr=path.join(ns,'candidate.json');const sha='a'.repeat(40);
  const ps=`$ErrorActionPreference='Stop';$env:AO_REVIEW_START_MONOTONIC_NOW_MS='1000';. '${psQuote(legacyScript)}';while(-not(Test-Path -LiteralPath '${psQuote(start)}')){Start-Sleep -Milliseconds 10};$r=Acquire-ReviewStartClaim -PrNumber 948 -HeadSha '${sha}' -Surface 'legacy-overlap' -Namespace '${psQuote(ns)}' -ReviewRuns @();$r|ConvertTo-Json -Compress -Depth 30|Set-Content -LiteralPath '${psQuote(lr)}' -Encoding utf8;while(-not(Test-Path -LiteralPath '${psQuote(release)}')){Start-Sleep -Milliseconds 10}`;
  const js=`import{existsSync,writeFileSync}from'node:fs';import{acquireReviewStartClaim}from ${JSON.stringify(pathToFileURL(path.join(input.repoRoot,'scripts/lib/review-start-claim-store.ts')).href)};const wait=p=>{while(!existsSync(p))Atomics.wait(new Int32Array(new SharedArrayBuffer(4)),0,0,10)};wait(${JSON.stringify(start)});const r=acquireReviewStartClaim({prNumber:948,headSha:'${sha}',surface:'candidate-overlap',namespace:${JSON.stringify(ns)},reviewRuns:[]});writeFileSync(${JSON.stringify(cr)},JSON.stringify(r));wait(${JSON.stringify(release)});`;
  ...
  const p1=waitChild(input.pwshPath,['-NoProfile','-Command',ps]);const p2=waitChild(input.nodePath,['--experimental-strip-types','--input-type=module','-e',js]);await new Promise(r=>setTimeout(r,80));writeFileSync(start,'go');const deadline=Date.now()+15000;while(!(existsSync(lr)&&existsSync(cr))){if(Date.now()>deadline)throw new Error('acquisition_result_timeout');await new Promise(r=>setTimeout(r,20))}writeFileSync(release,'release');await Promise.all([p1,p2]);
