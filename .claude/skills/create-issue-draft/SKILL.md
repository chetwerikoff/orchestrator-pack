...
description: Use for GPT-authored orchestrator-pack task specs. The GitHub Issue is the live spec. T1/T2 use one terminal GPT architectural source. T3 competitive and architectural-review each use exact independent 01/02/03 siblings in one triple-source/v1 stageAttemptId; Claude and terminal architectural remain singular. Stage receipts, verified relay equality, occurrence accounting, bounded zero-send retry, and the #1123 activation seam are binding.
---
...
- #1150: exact plural pre-terminal source sets, episode derivation, relay, and
  occurrence accounting;
- #1123: Issue-lifetime logical-round counting and activation of triple-source
  final acceptance.
...
`attemptOrdinal: 1`, `retryClass: eligible-zero-send`; retry is
`attemptOrdinal: 2`, `retryClass: retry`.
...
```text
competitive[01,02,03] → architectural-review[01,02,03] → architectural-lens (or valid waiver) → architectural
```

1. Intake and optional adjacent correction before first capture.
2. One exact `competitive` stage attempt with three independent GPT sources.
3. Author harvest/dispositions for the full governed stage union and M4 update.
4. One exact `architectural-review` stage attempt with three independent GPT
   sources.
...
files. Three sibling captures in one exact `stageAttemptId` are one logical round.
...
One `reviewEpisodeId` begins with the first selected reviewer-stage attempt after
intake correction closes and spans all stages, author-fix revisions, Claude,
terminal review, relay, and dispositions. It ends only as accepted, blocked, or
explicitly abandoned. It does not reset at lens/revision/chat/workdir boundaries.

Every stage attempt has one `stageAttemptId`, stage, policy, and frozen
`sourceRevision`. Cross-revision mixing inside one attempt or credentialing set
fails closed. Different valid attempts in the episode may bind later revisions;
all governed evidence remains in the episode union.

Only `stage-completeness-receipt/v1` is persisted as stage authority. Do not
persist an episode receipt. Both stage-completeness and finding-ledger guards call
the same pure `deriveReviewEpisodeState(stageReceipts, verifiedRelayEvidence)`.

The derivation exposes:

- per-stage credentialing capture sets;
- episode-wide governed and relayed capture unions;
- stage and episode raw occurrence counts;
- logical round identities (`stageAttemptId`);
- relay completeness and activation state.

## T3 triple-source attempt
...
2. Create exact reviewer slots `01`, `02`, `03` with independent reviewer-source,
...
6. Each invocation records immutable `reviewer-invocation-envelope/v1` facts:
   episode, attempt, policy, stage, source revision, slot/ordinal, reviewer source,
   invocation and terminal-result identity, attempt ordinal, terminal state and
   classification, `send_count`, retry class, revision check, observable capacity
   outcome/wait, and capture identity when complete.
...
Exact 01/02/03 complete siblings credential the stage. Missing, extra, duplicate,
...
Final acceptance supplies every episode stage receipt and the same episode-wide
verified relay evidence. Legacy unsuffixed directory scanning remains historical
read compatibility only; source-suffixed captures require explicit receipts.
...
not delivery. Corrected resends carry supersession identity.
...
occurrence maps exactly once to one author-owned distinct defect.

For each distinct defect record:

- canonical type without protected-type reclassification;
- one or more occurrence identities;
...
- existing machinery/M3 facts when applicable.

Record exact episode counts:

- `rawFindingCount`;
- `distinctFindingCount`;
- `processedDistinctCount`.

Any unresolved defect or count/mapping mismatch blocks. `NO_FINDINGS` from one
source never erases another source's occurrence.
...
After exact triple `competitive` and exact triple `architectural-review` are
settled, fully relayed, and dispositioned:

- union every `simplification-cut-candidate: yes` occurrence across the three
  architectural-review sources independent of file order;
- aggregate clean only when all three are locally `SIMPLIFICATION_CLEAN` and no
...
and M3. Normal post-Claude author fixes proceed to terminal GPT without a second
Claude lens.
...
Terminal GPT has full current-revision M3 authority under existing evidence and
why-now rules. Stale, malformed, duplicate-conflicting, or unresolved protected
state fails closed.

## Final acceptance

Supply all episode receipts and verified relay evidence to both guards. T1/T2 may
accept when their singular topology, ledger, body, tier, and M5 checks are green.
T3 final acceptance remains deliberately blocked until #1123 changes
Issue-lifetime round counting.

When activation is available, acceptance requires:

1. singular terminal GPT is sole M5 anchor;
2. exact plural pre-terminal T3 credentialing sets, singular Claude/waiver, and
...
5. exact raw/distinct/processed occurrence accounting with no unresolved defect;
6. Claude provenance when capture branch ran;
...
pass-NN-competitive-01.capture.txt
pass-NN-competitive-02.capture.txt
pass-NN-competitive-03.capture.txt
pass-NN-architectural-review-01.capture.txt
pass-NN-architectural-review-02.capture.txt
pass-NN-architectural-review-03.capture.txt
pass-NN-architectural-lens.capture.txt
pass-NN-architectural.capture.txt
<Claude producing-run evidence or unavailable waiver>
...
- Harvest/adjudicate a triple stage before all launches and settlement.
...
- Treat a Claude waiver as capture/M3 evidence.
