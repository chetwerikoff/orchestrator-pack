...
naive path). Your per-pass cost ≈ printed reply + reasoning. **Hard cap: 5 passes
...
- **5 passes done** — record still-open findings as explicit risks/open questions.
...
```
GPT loop: <N> passes; stopped because <no-accepted-finding-in-last-pass | cap-5>; last-pass accepted=<k>; final STATE=<state> VALIDATION=<v> pass=<PASS_ID> sha=<DRAFT_SHA256>
```
...
- **Exceed 5 passes**, or re-run with no accepted change.
