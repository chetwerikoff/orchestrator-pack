...
description: Use when the user asks for causes of a failure or recurrence — e.g. "разобраться с причиной", "в чём причина", "что это", "разберись", "почему упал", "что сломалось", "отладь", "что случилось", "почему не работает", "root cause", "why did", "figure out why", "investigate the cause", "wtf". Skip for pure implementation, external adoption (study-external-source), or when one tracked issue already fully answers the ask.
---
