...
PowerShell (append proposal to prompt; no stdin `<` redirect):
...
- **Hard cap: 3 cycles** (proposal → Codex → revise or rebut → repeat).
- Revise the proposal for valid findings; rebut wrong ones in the proposal.
- After cycle 3, stop revising — list remaining concerns as **open questions**.
...
4. **Open questions** — unresolved after 3 Codex cycles.
...
- Run more than **3** Codex iterations silently.
