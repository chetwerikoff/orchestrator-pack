...
CI verifies the Node 20 adapter path, the full light-lane cutover contract, strict self-architect policy, complete pack verification, and compatibility with the merged current-`main` gate-runner migration. The validation includes retirement of daemon reviewer-config writes.
