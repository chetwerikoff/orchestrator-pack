...
The same `pr2-scheduler` tick runs fleet supervision and the unsent Cursor composer pass concurrently. The composer pass immediately submits only a composer made entirely of one or more exact Orca mailbox-pointer lines; any human or mixed text is a no-effect. The tick waits for both passes to settle before preserving the fleet error, so a fleet-phase failure cannot cancel or skip the submit pass. Submitted fingerprints persist across tick processes. A crashed tick is restarted by the existing supervisor crash-backoff; do not add a second registry child or a long-lived CLI watcher beside a live supervisor. Manual/smoke CLI remains `node --experimental-strip-types scripts/cursor-unsent-composer-submit.ts` (`--once` for one scan).
