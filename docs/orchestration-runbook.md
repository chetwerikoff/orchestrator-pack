...
The same bounded scheduler child owns the fleet-sweep alarm request cadence. It derives the latest coordinator-addressed activity from the durable Run inbox, requires a live Dispatch or ready Task before sending, and requests a `Fleet sweep alarm` through the existing repository-governed orchestration-mail sender. The due re-check and send are serialized by the existing reconcile lock; after that lock is released, the sender performs the existing exact-message delivery companion, while ordinary orchestration-mail reconcile remains the maintenance path. Inbox creation is the durable request-cadence witness, not proof of recipient receipt, and the coordinator never arms its own timer.
