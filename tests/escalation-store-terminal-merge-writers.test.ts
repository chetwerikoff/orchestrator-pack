...
let root = '';
let matrix!: WriterMatrix;

beforeAll(() => {
  root = mkdtempSync(join(tmpdir(), 'escalation-terminal-writers-'));
...
describe('escalation-store actual writers and replay', () => {
  it('preserves an ACK written between snapshot and dirty writeback', () => expect(matrix).toMatchObject({ ackMidTickOk: true, ackMidTickState: 'acked' }));
  ...
  it('preserves a same-millisecond operator ACK and bookkeeping', () => expect(matrix).toMatchObject({ operatorWriterTieOk: true, operatorWriterTieStatus: 'acked', operatorWriterTieAtMs: 1100, operatorWriterTieUpdatedAtMs: 1100, operatorWriterTieAttempts: 5, operatorWriterTieFirstAttemptAtMs: 901 }));
  it('preserves a strictly newer operator ACK', () => expect(matrix).toMatchObject({ operatorWriterNewerOk: true, operatorWriterNewerStatus: 'acked', operatorWriterNewerAtMs: 1101, operatorWriterNewerUpdatedAtMs: 1101 }));
  it('uses the same invariant after replay and final writeback', () => expect(matrix).toMatchObject({ replayAckOk: true, replayTerminalState: 'acked' }));
  it('keeps ACKs written before snapshot and after writeback', () => expect(matrix).toMatchObject({ beforeAckOk: true, beforeState: 'acked', afterAckOk: true, afterState: 'acked' }));
});
