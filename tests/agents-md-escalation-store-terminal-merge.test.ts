...
describe('escalation-store terminal merge invariant', () => {
  ...
  it('enforces terminal, release, deletion, LWW, and operator-ack conflicts', () => {
    const result = runPwsh(`
      ...
      function Invoke-TestMerge {
        ...
        $merged = Merge-OrchestratorEscalationRouterWritebackState -State $memoryState -DiskState $diskState -DirtyRecordKeys $DirtyRecordKeys -SnapshotLoadedAtMs $SnapshotLoadedAtMs
        if (-not $merged.records.ContainsKey('alpha')) { return $null }
        return $merged.records['alpha']
      }

      $cases = @{}
      $cases.diskTerminalBeatsNewerOpen = Invoke-TestMerge -MemoryRecord (New-TestRecord -UpdatedAtMs 1300 -Attempts 4) -DiskRecord (New-TestRecord -TerminalState 'acked' -Status 'acked' -UpdatedAtMs 1100)
      $cases.diskTerminalWinsTie = Invoke-TestMerge -MemoryRecord (New-TestRecord -TerminalState 'dead_lettered' -Status 'dead_lettered' -UpdatedAtMs 1100) -DiskRecord (New-TestRecord -TerminalState 'acked' -Status 'acked' -UpdatedAtMs 1100)
      $cases.memoryTerminalWinsOnlyWhenStrictlyNewer = Invoke-TestMerge -MemoryRecord (New-TestRecord -TerminalState 'dead_lettered' -Status 'dead_lettered' -UpdatedAtMs 1101) -DiskRecord (New-TestRecord -TerminalState 'acked' -Status 'acked' -UpdatedAtMs 1100)
      $cases.freshReleaseWins = Invoke-TestMerge -MemoryRecord (New-TestRecord -TerminalState 'quarantined' -Status 'quarantined' -UpdatedAtMs 1300) -DiskRecord (New-TestRecord -UpdatedAtMs 1100 -ReleaseAtMs 1100) -SnapshotLoadedAtMs 1000
      $cases.sameMillisecondReleaseWins = Invoke-TestMerge -MemoryRecord (New-TestRecord -TerminalState 'quarantined' -Status 'quarantined' -UpdatedAtMs 1300) -DiskRecord (New-TestRecord -UpdatedAtMs 1000 -ReleaseAtMs 1000) -SnapshotLoadedAtMs 1000
      $cases.openWithoutReleaseMarkerLoses = Invoke-TestMerge -MemoryRecord (New-TestRecord -TerminalState 'quarantined' -Status 'quarantined' -UpdatedAtMs 1000) -DiskRecord (New-TestRecord -UpdatedAtMs 2000) -SnapshotLoadedAtMs 1000
      $cases.staleReleaseMarkerLoses = Invoke-TestMerge -MemoryRecord (New-TestRecord -TerminalState 'quarantined' -Status 'quarantined' -UpdatedAtMs 1000) -DiskRecord (New-TestRecord -UpdatedAtMs 2000 -ReleaseAtMs 999) -SnapshotLoadedAtMs 1000
      $cases.deleted = Invoke-TestMerge -MemoryRecord (New-TestRecord -UpdatedAtMs 1200) -DiskRecord $null
      $cases.nonterminalTieKeepsMemory = Invoke-TestMerge -MemoryRecord (New-TestRecord -UpdatedAtMs 1100 -Attempts 4) -DiskRecord (New-TestRecord -UpdatedAtMs 1100 -Attempts 2)
      $cases.newerDiskNonterminalWins = Invoke-TestMerge -MemoryRecord (New-TestRecord -UpdatedAtMs 1100 -Attempts 4) -DiskRecord (New-TestRecord -UpdatedAtMs 1101 -Attempts 2)
      $cases.tickDeadLetterPersists = Invoke-TestMerge -MemoryRecord (New-TestRecord -TerminalState 'dead_lettered' -Status 'dead_lettered' -UpdatedAtMs 1100) -DiskRecord (New-TestRecord -UpdatedAtMs 1000)
      $memoryAckTie = New-TestRecord -UpdatedAtMs 1200 -Attempts 5
      $memoryAckTie.firstAttemptAtMs = 901
      $cases.operatorAckTie = Invoke-TestMerge -MemoryRecord $memoryAckTie -DiskRecord (New-TestRecord -UpdatedAtMs 1200 -Attempts 2 -OperatorStatus 'acked' -OperatorAckedAtMs 1200)
      $cases.nonDirtyMidTick = Invoke-TestMerge -MemoryRecord (New-TestRecord -UpdatedAtMs 1300 -Attempts 9) -DiskRecord (New-TestRecord -TerminalState 'acked' -Status 'acked' -UpdatedAtMs 1100) -DirtyRecordKeys @()
  ...
  it('preserves real ACK writers before, during, and after a router tick', () => {
    ...
    const result = runPwsh(`
      ...
      $beforeMerged = Merge-OrchestratorEscalationRouterWritebackState -State $beforeSnapshot -DiskState $beforeDisk -DirtyRecordKeys @('alpha') -SnapshotLoadedAtMs 1200
  ...
  it('uses the same invariant for replay rereads and final dirty writeback', () => {
    ...
    const result = runPwsh(`
      ...
      $final = Merge-OrchestratorEscalationRouterWritebackState -State $tickMemory -DiskState $finalDisk -DirtyRecordKeys @('alpha') -SnapshotLoadedAtMs 1000
