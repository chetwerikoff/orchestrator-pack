...
let root = '';
let matrix!: MergeMatrix;

beforeAll(() => {
  root = mkdtempSync(join(tmpdir(), 'escalation-terminal-merge-'));
...
describe('escalation-store conflict-aware writeback', () => {
  ...
  it('preserves a terminal record written by the tick itself', () => {
    expect(matrix.tickTerminal).toBe('dead_lettered');
  });

  it('preserves a non-dirty out-of-band terminal write', () => {
    expect(matrix.nonDirtyTerminal).toBe('acked');
  });
  ...
  it('preserves an actual quarantine release', () => {
    expect(matrix.releaseWriterState).toBe('open');
  });

  it('preserves an actual quarantine deletion', () => {
    expect(matrix.deleteWriterMissing).toBe(true);
  });

  it('preserves an actual same-millisecond operator ACK and its writer timestamp', () => {
    expect(matrix).toMatchObject({
  ...
  it('preserves a strictly newer operator ACK and updatedAtMs bump', () => {
    expect(matrix).toMatchObject({
