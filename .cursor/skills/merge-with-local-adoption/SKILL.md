...
description: >-
  ...
  orchestrator runs on the current commit after operator restart. After merge,
  kill the merged PR's worker AO session and run ao session cleanup -p
  ...
  clearly wants a ready PR merged after review/CI. Operates on the operator's
  live working tree in Cursor; never discards uncommitted local work. Skip when
  the user only asks about merge policy without a concrete PR.
---
