...
describe('tracked wrapper hostname transport', () => {
  it('does not pass --hostname to the native PR-head REST transport', () => {
    const root = mkdtempSync(join(tmpdir(), 'gh-hostname-'));
    const native = join(root, 'native-gh');
    writeExecutable(native, `#!/usr/bin/env bash
set -euo pipefail
for arg in "$@"; do
  [[ "$arg" != "--hostname" ]] || { echo 'bad option: --hostname' >&2; exit 2; }
done
[[ "\${1:-}" == api ]] || exit 3
printf '%s\\n' '{"number":1698,"head":{"sha":"48365ebdc83c838c24b2b6452e43f5840b7c7b33"}}'
`);
    try {
      const result = ghApiJson(native, 'repos/chetwerikoff/orchestrator-pack/pulls/1698', { cwd: root });
