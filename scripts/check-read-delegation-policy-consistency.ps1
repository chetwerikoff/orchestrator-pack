...
$new = '$leaks = @(Get-Issue771DependencyScopeLeaks -RepositoryRoot $RepoRoot -ScanRoot (Join-Path $RepoRoot ''scripts'') | Where-Object { $_.LoaderPath -match ''(WorkerStatusStore|Get-WorkerStatusDecisionSessions|Autonomous-ReviewWorktreeGate|TestMode-FleetLease)\.ps1$'' })'
...
Write-Host ("ISSUE771_RCA_CLASSIFICATION total={0} passed={1} failed={2}" -f $result.TotalCount, $result.PassedCount, $result.FailedCount)
