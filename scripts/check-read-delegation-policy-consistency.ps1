...
$new = '$leaks = @(Get-Issue771DependencyScopeLeaks -RepositoryRoot $RepoRoot -ScanRoot (Join-Path $RepoRoot ''scripts'') | Where-Object { [IO.Path]::GetFileName($_.LoaderPath) -match ''^[A-Fa-f]'' })'
...
Write-Host ("ISSUE771_A_F_CLASSIFICATION total={0} passed={1} failed={2}" -f $result.TotalCount, $result.PassedCount, $result.FailedCount)
