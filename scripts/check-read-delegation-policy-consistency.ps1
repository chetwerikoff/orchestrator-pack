...
$new = '$leaks = @(Get-Issue771DependencyScopeLeaks -RepositoryRoot $RepoRoot -ScanRoot (Join-Path $RepoRoot ''scripts'') | Where-Object { $_.LoaderFunction -eq ''Get-ClaimedReviewStartSnapshot'' -and $_.ConsumerFunction -match ''^(?i)Get-'' })'
...
Write-Host ("ISSUE771_CLAIMED_GET_CONSUMER total={0} passed={1} failed={2}" -f $result.TotalCount, $result.PassedCount, $result.FailedCount)
