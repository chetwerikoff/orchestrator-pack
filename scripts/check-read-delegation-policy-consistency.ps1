...
try {
    ...
    $new = '$leaks = @(Get-Issue771DependencyScopeLeaks -RepositoryRoot $RepoRoot -ScanRoot (Join-Path $RepoRoot ''scripts'') | Where-Object { $_.LoaderFunction -match ''^(?i)Get-Review(?:Recovery|Run|Post)'' })'
