...
export const RUNTIME_CALLER_CENSUS: readonly RuntimeCallerCensusRow[] = [
  ...
  },
  {
    ...
    currentConsumer: 'worker-smoke CLI and its run-level control-plane matrix',
    removalOwner: '#1248 caller cut after generation-aware fixture and exact consumer migration',
    note: 'The caller remains on the native compatibility seam; switching it without a runtime incarnation would weaken destructive-operation safety.',
  },
  {
    ...
    kind: 'runtime-port',
    disposition: 'legacy-only',
    currentConsumer: 'scripts/worker-smoke-run.ts',
    removalOwner: '#1248 worker-smoke caller cut',
    note: 'The bounded native create seam remains until worker-smoke receives an exact runtime generation and moves as one lifecycle unit.',
  },
  {
