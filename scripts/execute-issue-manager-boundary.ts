...
import { evaluateCreateIssueManagerBoundary } from './lib/create-issue-manager-boundary.ts';
import {
...
function usage(): string {
  return [
    'Execute-Issue manager result boundary',
    '',
    'Usage:',
...
function parsePhase(value: string | undefined): ExecuteIssuePhase {
  if (!value || !EXECUTE_ISSUE_PHASES.includes(value as ExecuteIssuePhase)) {
    throw new Error('--phase must be one of ' + EXECUTE_ISSUE_PHASES.join(', '));
  }
...
function defectFromCliError(error: unknown, currentArgv: readonly string[]) {
  return evaluateCreateIssueManagerBoundary({
    producer: 'execute-issue-manager-boundary.ts:cli',
    currentArgv,
    produce: () => {
...
export function runExecuteIssueManagerBoundaryCli(
  ...
  try {
    ...
    const evaluated = classifyExecuteIssueManagerRecord(input, {
      ...parsed.context,
      currentArgv,
    });
...
if (direct) {
  process.exitCode = runExecuteIssueManagerBoundaryCli(process.argv.slice(2));
}
