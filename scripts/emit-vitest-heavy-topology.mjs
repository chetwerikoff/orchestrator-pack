...
const child = spawnSync('pwsh', ['-NoProfile', '-File', 'scripts/check-ci-pipeline-split.ps1'], {
  cwd: repoRoot,
  encoding: 'utf8',
  timeout: 600_000,
...
const artifact = {
  heavyShardCount: 1,
  heavyShardMatrix: [1],
  fallbackClassification: 'pr768-ci-pipeline-split-diagnostic',
  discovered: [],
  fullDiscovered: [],
  heavyFiles: [],
  lightFiles: [],
  ...
  ciPipelineSplitDiagnostic: {
    status: child.status,
    signal: child.signal,
    error: child.error?.message ?? null,
    tail: lines.slice(-400),
  },
};
...
if (process.argv.includes('--gha-output')) {
  ...
  appendFileSync(outputPath, 'fallback_classification=pr768-ci-pipeline-split-diagnostic\n');
}
console.log(JSON.stringify({ ...artifact, ciPipelineSplitDiagnostic: { ...artifact.ciPipelineSplitDiagnostic, tail: `${artifact.ciPipelineSplitDiagnostic.tail.length} lines captured in artifact` } }));
