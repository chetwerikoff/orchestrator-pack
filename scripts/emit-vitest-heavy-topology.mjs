#!/usr/bin/env node
import { appendFileSync, writeFileSync } from 'node:fs';
import { join } from 'node:path';
import { runProcess } from './kernel/subprocess.mjs';

const repoRoot = process.cwd();
const runner = join(repoRoot, 'scripts', 'run-vitest-with-harness.mjs');
const cases = [
  {
    file: 'plugins/ao-codex-pr-reviewer/tests/reviewer-budget.test.ts',
    timeoutSeconds: 240,
  },
  {
    file: 'scripts/github-fleet-repo-tick-snapshot.test.ts',
    timeoutSeconds: 720,
  },
];
const MAX_CAPTURE = 240_000;

function bounded(value) {
  const text = String(value ?? '');
  return text.length > MAX_CAPTURE ? text.slice(-MAX_CAPTURE) : text;
}

async function runCase(entry) {
  const startedAt = Date.now();
  const result = await runProcess({
    command: 'timeout',
    args: [
      '--signal=TERM',
      '--kill-after=5s',
      `${entry.timeoutSeconds}s`,
      process.execPath,
      runner,
      'run',
      entry.file,
      '--reporter=verbose',
      '--maxWorkers=1',
    ],
    cwd: repoRoot,
    inheritParentEnv: true,
    env: {
      CI: 'true',
      OPK_TESTMODE_FLEET_WORKSPACE_ROOT: repoRoot,
    },
  });
  const timedOut = result.exitCode === 124 || result.exitCode === 137;
  const record = {
    file: entry.file,
    timeoutSeconds: entry.timeoutSeconds,
    outcome: result.outcome,
    status: result.exitCode,
    signal: result.signal,
    timedOut,
    durationMs: Date.now() - startedAt,
    error: result.error ?? null,
    stdout: bounded(result.stdout),
    stderr: bounded(result.stderr),
  };
  console.log(
    JSON.stringify({
      file: record.file,
      status: record.status,
      signal: record.signal,
      timedOut,
      durationMs: record.durationMs,
    }),
  );
  return record;
}

const results = [];
for (const entry of cases) {
  results.push(await runCase(entry));
}

const failures = results.filter(
  (result) => result.status !== 0 || result.timedOut || result.error,
);
const artifact = {
  schemaVersion: 1,
  diagnostic: 'issue-885-shard7-serial-isolation',
  head: process.env.GITHUB_SHA ?? null,
  heavyShardCount: 0,
  heavyShardMatrix: [],
  fallbackClassification: 'diagnostic',
  results,
};

writeFileSync(
  join(repoRoot, 'scripts', 'vitest-heavy-topology.plan.json'),
  `${JSON.stringify(artifact, null, 2)}\n`,
);

if (process.env.GITHUB_OUTPUT) {
  appendFileSync(
    process.env.GITHUB_OUTPUT,
    'heavy_shard_count=0\nheavy_shard_matrix=[]\nfallback_classification=diagnostic\n',
  );
}

console.log(
  JSON.stringify({
    diagnostic: artifact.diagnostic,
    failures: failures.map((result) => result.file),
  }),
);
