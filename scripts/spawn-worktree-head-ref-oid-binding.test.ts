...
import { buildSpawnWorktreeGrantRecord, evaluateSpawnWorktreeGrantConsume } from '../docs/spawn-worktree-grant.mjs';
import { evaluateSpawnClaimPrPostCheckout, evaluateSpawnWorktreeHeadRefAuthorization, resolveGitCommitRefInRepo, resolveSpawnDefaultBranchBaseRef, rewriteGitWorktreeAddCommitArgv } from '../docs/spawn-worktree-git-ref.mjs';
...
const captureManifestPath = path.join(repoRoot, 'tests/external-output-references/captures/spawn-worktree-head-ref-oid-binding/capture-manifest.json');
...
function setupSpawnRepo(run: (ctx: {
    repo: string;
    mainOid: string;
    baseRef: string;
}) => void) {
...
describe('spawn worktree head-ref OID binding (#493)', () => {
    ...
    it('rewrites worktree add argv to normalized full OID (mutable-ref race closure)', () => {
        setupSpawnRepo(({ repo, mainOid, baseRef }) => {
            const rewritten = rewriteGitWorktreeAddCommitArgv(['worktree', 'add', '-b', 'opk-493', '/tmp/projects/orchestrator-pack/worktrees/opk-493', baseRef], mainOid);
