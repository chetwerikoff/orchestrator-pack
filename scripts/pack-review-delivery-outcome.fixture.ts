...
describe('pack review delivery outcome linkage (Issue #862)', () => {
  ...
  it('is mechanically loaded by the required review-delivery light suite', () => {
    const config = JSON.parse(
      readFileSync(path.join(repoRoot, 'scripts/vitest-ci-lanes.config.json'), 'utf8'),
    ) as { classification?: Record<string, string> };
    const suite = readFileSync(path.join(repoRoot, 'scripts/review-delivery.test.ts'), 'utf8');
    expect(config.classification?.['scripts/review-delivery.test.ts']).toBe('light');
    expect(suite).toContain("import './pack-review-delivery-outcome.fixture.js';");
    expect(suite).toContain("import './review-delivery-outcome-runner.fixture.js';");
  });
