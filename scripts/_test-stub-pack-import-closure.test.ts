...
import { assertStubPackDocsImportClosure } from './_test-stub-pack-import-closure.js';
describe('stub-pack docs import-closure guard (#508)', () => {
    it('fails with (fixture site, module, missingDep) for synthetic missing sibling import', () => {
        ...
        try {
            expect(() => assertStubPackDocsImportClosure('synthetic-missing-sibling', packRoot)).toThrowError(/stub-pack docs import-closure failed \(synthetic-missing-sibling, closure-probe-entry\.mjs, synthetic-missing\.mjs\)/);
        }
        finally {
    ...
    it('times out cleanly for a non-import-safe module without hanging the vitest worker', () => {
        ...
        const docsDir = path.join(packRoot, 'docs');
        mkdirSync(docsDir, { recursive: true });
        writeFileSync(path.join(docsDir, 'stdin-hang-probe.mjs'), 'while (true) {}\n');
        try {
            expect(() => assertStubPackDocsImportClosure('stdin-hang-probe', packRoot, { timeoutMs: 500 })).toThrow(/stub-pack docs import-closure timed out \(stdin-hang-probe\)/);
        }
        finally {
            rmSync(packRoot, { recursive: true, force: true });
        }
