...
describe('orchestrator-wake-supervisor lifecycle', () => {
  it(
    'stops the session-bound child when orchestrator session disappears at runtime',
    async () => {
      ...
      await ows.waitForMarkers(stateDir, 25_000, [sessionBoundRole]);
  ...
  it(
    ...
    async () => {
      ...
      await ows.waitForMarkers(stateDir, 25_000, [genericChildRole, sessionBoundRole]);
  ...
  it(
    ...
    async () => {
      ...
      const genericBefore = await ows.readMarker(stateDir, genericChildRole);
      const routerBefore = await ows.readMarker(stateDir, sessionBoundRole);
      expect(ows.isAlive(genericBefore.pid)).toBe(true);
      expect(ows.isAlive(routerBefore.pid)).toBe(true);

      const stop = ows.runSupervisor(['-Action', 'Stop', '-StateDir', stateDir]);
      expect(stop.status).toBe(0);

      await ows.waitForProcessesStopped([genericBefore.pid, routerBefore.pid], 1500);
      expect(ows.isAlive(genericBefore.pid)).toBe(false);
      expect(ows.isAlive(routerBefore.pid)).toBe(false);
  ...
  it('status exits non-zero when supervisor is stopped but a child remains', async () => {
    const stateDir = ows.makeStateDir();
    const child = ows.startSupervisorBackground(stateDir, [
      '-OrchestratorSessionId',
      'op-status-orphan',
    ]);
    await ows.waitForMarkers(stateDir, 25_000, [genericChildRole]);

    const supervisorPid = Number(
      ows.fs.readFileSync(ows.path.join(stateDir, 'supervisor.pid'), 'utf8').trim(),
    );
    const generic = await ows.readMarker(stateDir, genericChildRole);
    child.kill('SIGTERM');
    await ows.waitForProcessesStopped([supervisorPid], 500);
  ...
  it(
    ...
    async () => {
      ...
      try {
        await ows.readMarker(stateDir, genericChildRole, 20_000);
      } catch {
      ...
      while (Date.now() < deadline) {
        try {
          const marker = await ows.readMarker(stateDir, genericChildRole, 500);
        ...
        if (ows.fs.existsSync(logPath)) {
          ...
          if (/crash backoff: review-trigger-reconcile/.test(supervisorLog)) {
            break;
          }
        }
