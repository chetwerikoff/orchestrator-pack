...
function initRepo(prefix) {
  ...
  git(root, ['config', 'user.name', 'merge-blind-fixture']);
  return root;
}
...
function prepareGateRepo() {
  const root = initRepo('opk-823-gates-');
  write(
...
`,
  ...
  const base = commitAll(root, 'semantic prerequisites');
...
function verifySharedBaseParity(gateRepo) {
  const pr = withProcessEnv(contextEnv('pr', gateRepo.base), () => resolveMergeStableCiBase(gateRepo.root));
  const push = withProcessEnv(contextEnv('push'), () => resolveMergeStableCiBase(gateRepo.root));
  ...
  assert.deepEqual(
    git(gateRepo.root, ['diff', '--name-only', `${pr.baseSha}...HEAD`]).split('\n').filter(Boolean),
    git(gateRepo.root, ['diff', '--name-only', `${push.baseSha}...HEAD`]).split('\n').filter(Boolean),
    'PR/push changed-file sets must match',
  );
}

function verifyPowerShellGateParity(gateRepo) {
  const probe = run('pwsh', ['-NoProfile', '-Command', '$PSVersionTable.PSVersion.Major']);
  if (probe.status !== 0) {
  ...
  expectSameStatus(
    'sequencing positive',
    run('pwsh', ['-NoProfile', '-File', sequencing, gateRepo.root], { env: prEnv }),
    run('pwsh', ['-NoProfile', '-File', sequencing, gateRepo.root], { env: pushEnv }),
    0,
  );
  expectSameStatus(
    'message registry positive',
    run('pwsh', ['-NoProfile', '-File', messageRegistry, gateRepo.root], { env: prEnv }),
    run('pwsh', ['-NoProfile', '-File', messageRegistry, gateRepo.root], { env: pushEnv }),
    0,
  );
  ...
  commitAll(gateRepo.root, 'genuine binding drift');
  git(gateRepo.root, ['update-ref', 'refs/remotes/origin/main', 'HEAD']);
...
function verifyRpcBindingParity() {
  const root = initRepo('opk-823-rpc-');
  ...
  const capture = commitAll(root, 'capture scoped tree');
  ...
  commitAll(root, 'bind metadata to capture');
  write(join(root, 'unrelated.txt'), 'merge identity changes, scope does not\n');
  commitAll(root, 'unrelated commit');

  const pr = withProcessEnv(contextEnv('pr', capture), () => inspectSupervisorHeavyLaneRpcBinding(root));
  const push = withProcessEnv(contextEnv('push'), () => inspectSupervisorHeavyLaneRpcBinding(root));
  ...
  commitAll(root, 'genuine RPC scope drift');
  const prNegative = withProcessEnv(contextEnv('pr', capture), () => inspectSupervisorHeavyLaneRpcBinding(root));
  const pushNegative = withProcessEnv(contextEnv('push'), () => inspectSupervisorHeavyLaneRpcBinding(root));
...
const gateRepo = prepareGateRepo();
try {
  verifySharedBaseParity(gateRepo);
  verifyPowerShellGateParity(gateRepo);
  verifyRpcBindingParity();
  console.log(
