...
function mutationFailures(key:string, artifact:string):string[]{
  ...
  const activate=mbody(F.tx,'export async function activateCutover'), recover=mbody(F.recovery,'export async function recoverCommittedCutover');
  ...
  const recoveryText=mread(F.recovery); need(recoveryText.indexOf('authority.commit(request.expectedOldEpochId, core);')>=0&&recoveryText.indexOf('boundary.ensureTypeScriptSupervisor(')>recoveryText.indexOf('authority.commit(request.expectedOldEpochId, core);'),'recovery:precas');
