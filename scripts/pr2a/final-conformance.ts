...
function existsAt(ref: string, file: string): boolean {
  return runProcessSync({ command: 'git', args: ['cat-file', '-e', `${ref}:${file}`], cwd: repoRoot, inheritParentEnv: true }).ok;
}
...
export function scanForbiddenExecutableReferences(files: Array<{ path: string; content: string }>): ConformanceFinding[] {
  ...
  for (const row of files) {
    ...
    if (!governanceReference(row.path)) findings.push({ code: 'd928_external_executable_reference', path: row.path, detail: matched.join(',') });
  }
  return findings;
}
...
export function validateClaimStoreSource(source: string): ConformanceFinding[] {
  ...
  for (const token of required) {
    if (!source.includes(token)) findings.push({ code: 'claim_store_protocol_guard_missing', path: 'scripts/lib/review-start-claim-store.ts', detail: token });
  }
...
export function validateClosureReceiptSource(source: string): ConformanceFinding[] {
  ...
  for (const token of required) {
    if (!source.includes(token)) findings.push({ code: 'closure_receipt_independent_verification_missing', path: 'scripts/pr2a/closure-receipt.ts', detail: token });
  }
...
export function validateMandatoryPackageScripts(source: string): ConformanceFinding[] {
  ...
  try { parsed = JSON.parse(source) as { scripts?: Record<string, string> }; }
  catch { return [{ code: 'package_scripts_unreadable', path: 'package.json' }]; }
...
function validateOperationSet(manifest: PlanningManifest, rows: DiffRow[]): ConformanceFinding[] {
  ...
  for (const [file, operation] of expected) {
    if (actual.get(file) !== operation) findings.push({ code: 'planned_operation_missing_or_changed', path: file, detail: `expected=${operation};actual=${actual.get(file) ?? 'none'}` });
  }
  for (const [file, operation] of actual) if (!expected.has(file)) findings.push({ code: 'unreviewed_final_tree_operation', path: file, detail: operation });
  return findings;
}
...
function validateLiveManifests(ref: string): ConformanceFinding[] {
  ...
  for (const file of liveReferenceManifests) {
    ...
    for (const basename of basenames) if (source.includes(basename)) findings.push({ code: 'actionable_manifest_retains_d928', path: file, detail: basename });
  }
  return findings;
}
...
export function buildConformanceReport(ref = 'HEAD'): ConformanceReport {
  ...
  if (git(['rev-parse', `${manifest.lineage.planningCommit}^{tree}`]).trim() !== manifest.lineage.planningBaseTreeOid) findings.push({ code: 'planning_tree_binding_mismatch' });
  ...
  findings.push(...validateClaimStoreSource(readAt(ref, 'scripts/lib/review-start-claim-store.ts')));
  findings.push(...validateClosureReceiptSource(readAt(ref, 'scripts/pr2a/closure-receipt.ts')));
  findings.push(...validateMandatoryPackageScripts(readAt(ref, 'package.json')));
  ...
  const results = {
    ...
    AC2: acResult(findings, ['runner_', 'claim_store_']),
    AC3: acResult(findings, ['bridge_', 'claim_internal_', 'actionable_manifest_', 'd928_external_']),
    AC4: acResult(findings, ['d928_test_', 'd928_bytes_', 'd928_target_']),
    AC5: acResult(findings, ['bridge_', 'runner_', 'claim_store_', 'claim_internal_', 'closure_receipt_', 'd928_external_']),
    ...
    AC8: acResult(findings, ['package_', 'issue948_', 'contract_mutation_', 'closure_receipt_', 'claim_store_', 'bridge_', 'runner_', 'claim_internal_', 'd928_', 'planning_', 'planned_', 'unreviewed_', 'path_outside_', 'denylisted_', 'new_powershell_', 'non_regular_', 'retired_launch_', 'actionable_manifest_']),
  };
  return {
    schemaVersion: 1,
    issue: 948,
