...
type DetectorPattern = 'foundation' | 'admission-guards' | 'closure' | 'activation' | 'ordering' | 'watermark' | 'writer-survivor' | 'survivor' | 'recovery' | 'rollback' | 'import-guards' | 'scope' | 'vectors' | 'node' | 'platform' | 'canonical-root' | 'primitives' | 'registry' | 'scheduler';
...
interface MutationEvidence {
  ac: AcceptanceId; mutationId: string; artifactPath: string; detectorId: string; detectorCommand: string[];
  artifactHashBefore: string; artifactHashAfter: string; restoredHash: string;
  negativeOutcome: 'failed'; restoredOutcome: 'passed'; negativeExitCode: number; restoredExitCode: 0;
}
...
function snapshotArtifact(relativePath: string): ArtifactSnapshot {
  ...
  return existsSync(file) ? { existed: true, bytes: readFileSync(file), mode: statSync(file).mode & 0o777 } : { existed: false, bytes: Buffer.alloc(0), mode: 0o600 };
}
...
function replaceSpec(artifactPath: string, detector: DetectorPattern, token: string, replacement: string, all = false): MutationSpec {
  return { artifactPath, detector, apply: (snapshot) => {
    ...
    return { bytes: Buffer.from(all ? source.split(token).join(replacement) : source.replace(token, replacement), 'utf8'), mode: snapshot.mode };
  } };
}
...
function registrySpec(): MutationSpec {
  return replaceSpec('scripts/orchestrator-side-process-registry.cutover-target.json', 'registry', '"id": "pr2-scheduler"', '"id": "legacy-mutation"');
}
...
function mutationSpec(ac: AcceptanceId, id: string): MutationSpec {
  if (ac === 'AC1') {
    ...
    if (id === 'closure-unresolved-set-nonempty') return replaceSpec('scripts/lib/cutover/activation-transaction.ts','admission-guards',"throw new Error('closure_unresolved_set_nonempty');","throw new Error('mutation_unresolved_guard_removed');");
    ...
    if (id === 'host-or-repo-unbound' || id === 'old-installed-revision-missing') return replaceSpec('scripts/lib/cutover/activation-platform-preflight.ts','canonical-root','if (value !== lexical || lexical !== canonical)','if (false)');
    ...
    if (id === 'mutation-before-admission') return insertBeforeSpec('scripts/lib/cutover/activation-transaction.ts','ordering','const preflight = boundary.preflight(request);',"writeFileSync(request.paths.followupPath, 'mutation-before-admission');");
  ...
  if (ac === 'AC3') {
    ...
    if (id === 'snapshot-digest-not-raw-bytes') return replaceSpec('scripts/lib/cutover/activation-import.ts','import-guards','snapshotDigest: sha256Bytes(bytes)',"snapshotDigest: 'sha256:mutation'");
    ...
    if (id === 'store-covered-field-omitted') return replaceSpec('scripts/lib/cutover/activation-import.ts','import-guards',"throw new Error(`store_missing_field:${spec.id}:${key}`)","throw new Error('mutation_missing_field_guard')");
    if (id === 'unknown-store-field-silently-ignored') return replaceSpec('scripts/lib/cutover/activation-import.ts','import-guards',"throw new Error(`store_unknown_field:${spec.id}:${unknown.join(',')}`)","throw new Error('mutation_unknown_field_guard')");
  ...
  if (ac === 'AC4') {
    ...
    return appendSpec('scripts/lib/cutover/activation-transaction.ts','admission-guards',`// mutation successor ordering: ${id}`);
  }
  ...
  if (ac === 'AC7') {
    ...
    if (/followup-sequence|followup-epoch-reference|followup-treated|followup-missing|scheduler-enable-followup|ts-supervisor-start-followup/.test(id)) return replaceSpec('scripts/lib/cutover/activation-evidence.ts','activation','    sequence: existing.length + 1,','    sequence: existing.length + 2,');
    ...
    if (id === 'pr2a-closure-admission-evidence-missing' || id === 'ts-supervisor-inert-evidence-missing') return replaceSpec('scripts/lib/cutover/activation-transaction.ts','admission-guards','const foundation = boundary.proveFoundationAdoption(request);',"const foundation = { result: 'foundation-evidence-verified' } as FoundationAdmissionProof;");
...
async function main(): Promise<void> {
  ...
  const detectors = [...new Set(planned.map((row) => row.spec.detector))];
