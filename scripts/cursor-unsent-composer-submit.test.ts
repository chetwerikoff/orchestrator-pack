...
describe('model-family footer matrix', () => {
  ...
  it('does not record pointer_consumed when a pointer-visible episode is later unrecognized', async () => {
    const target = worker('term_unrecognized_after_visible');
    const message = {
      id: 'msg_unrecognized_after_visible',
      runId: 'run_unrecognized_after_visible',
      recipient: target.identity.id,
      consumed: false,
    };
    const key = `${workerKey(target.identity)}\u0000message\u0000${message.id}`;
    const state = {
      ...
      episodes: {
        [key]: {
          ...
          nextEligibleAt: 0,
          backoffMs: 1_000,
          state: 'pointer-visible' as const,
        },
      },
    };
    const submitted: RuntimeWorkerIdentity[] = [];
    const result = await submitOrcaMessageDeliveryPointer(message.id, {
      ...
      submitDeps: depsFor({}, {
        ...
        read: () => ({
          ok: true as const,
          lines: ['Ship the draft - 100% - Run Everything', cwd],
          source: 'screen' as const,
        }),
      }),
      episodeState: state,
    });
    expect(result.terminals[0]?.reason).toBe('composer_preview_unrecognized');
    expect(submitted).toEqual([]);
    expect(state.episodes[key]?.state).not.toBe('confirmed');
  });

  it('does not record pointer_consumed when a recognized placeholder hides a queued pointer and Enter was not sent', async () => {
    const target = worker('term_follow_up_queue');
    const message = {
      id: 'msg_follow_up_queue',
      runId: 'run_follow_up_queue',
      recipient: target.identity.id,
      consumed: false,
    };
    const key = `${workerKey(target.identity)}\u0000message\u0000${message.id}`;
    ...
    const result = await submitOrcaMessageDeliveryPointer(message.id, {
      ...
      submitDeps: depsFor({}, {
        ...
        read: () => ({
          ...
          lines: [
            '┌ follow-ups ┐',
            '+12 more lines · enter steer · ↑ select/edit · esc cancel',
            '→ Add a follow-up',
            ...CURSOR_FOOTER,
          ],
          source: 'screen' as const,
    ...
    expect(result.terminals[0]?.reason).toBe('submission_unconfirmed');
    expect(result.terminals[0]?.enter).toBe(false);
    expect(submitted).toEqual([]);
