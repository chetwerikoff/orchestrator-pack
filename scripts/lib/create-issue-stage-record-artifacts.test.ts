...
describe('Issue #2017 bind-published-comment missing slot row', () => {
  ...
  function writeEvidence(rows: Record<string, unknown>[]): string {
    const dir = mkdtempSync(join(tmpdir(), 'opk-2017-bind-'));
    tempDirs.push(dir);
    const evidencePath = join(dir, 'attempt-001.json');
  ...
  function commentTransport(body: string) {
    const calls: string[][] = [];
    return {
      calls,
      transport: {
  ...
  it('creates one row for an empty slot and does not send a Browser-GPT turn', () => {
    const evidencePath = writeEvidence([invocation('02', 'invocation-slot-02')]);
    const otherBefore = JSON.parse(readFileSync(evidencePath, 'utf8')).invocations[0];
    const createdId = 'invocation-slot-01';
    const { transport, calls } = commentTransport(headerFor(createdId));
    const result = bindPublishedCommentToSlot({
      reviewDir: tempDirs.at(-1)!,
    ...
    expect(stored.invocations.filter((row) => row.reviewerSlot === '01')).toEqual([
      expect.objectContaining({
        invocationId: createdId,
        sendCount: 1,
        artifactAuthority: expect.objectContaining({ commentId, commentUrl }),
      }),
    ]);
    expect(stored.invocations.find((row) => row.reviewerSlot === '02')).toEqual(otherBefore);
  });

  it('fails closed on a different invocation id and writes nothing', () => {
    const evidencePath = writeEvidence([invocation('01', invocationId)]);
    const before = readFileSync(evidencePath, 'utf8');
    const { transport } = commentTransport(headerFor('invocation-other'));
    const result = bindPublishedCommentToSlot({
      reviewDir: tempDirs.at(-1)!,
      stageEvidencePath: evidencePath,
      repositoryFullName: REPOSITORY,
    ...
    expect(result.ok).toBe(false);
    expect(result.errors.join('\n')).toContain('does not match --invocation-id');
    expect(readFileSync(evidencePath, 'utf8')).toBe(before);
  });

  it('updates a matching row in place without adding a second row', () => {
    const evidencePath = writeEvidence([invocation('01', invocationId), invocation('02', 'invocation-slot-02')]);
    const otherBefore = JSON.parse(readFileSync(evidencePath, 'utf8')).invocations[1];
    const { transport } = commentTransport(headerFor(invocationId));
    const result = bindPublishedCommentToSlot({
      reviewDir: tempDirs.at(-1)!,
      stageEvidencePath: evidencePath,
      repositoryFullName: REPOSITORY,
    ...
    expect(stored.invocations[0]).toMatchObject({ invocationId, sendCount: 1, retryClass: 'retry-forbidden' });
    expect(stored.invocations[1]).toEqual(otherBefore);
  });
});
