  ...
  foreach ($command in @($ast.FindAll({ param($node) $node -is [System.Management.Automation.Language.CommandAst] }, $true))) {
    ...
    if ($assignment -and $assignment.Left -is [System.Management.Automation.Language.VariableExpressionAst]) {
      ...
      foreach ($parser in $jsonParsers) {
        $parserContext = $parser.Parent
        while ($parserContext -and -not ($parserContext -is [System.Management.Automation.Language.PipelineAst])) { $parserContext = $parserContext.Parent }
        $parserText = if ($parserContext) { $parserContext.Extent.Text } else { $parser.Extent.Text }
        if ($parser.Extent.StartOffset -gt $assignment.Extent.EndOffset -and $parserText -match ('(?i)\$' + [regex]::Escape($variable) + '\b')) {
          $parsedLater = $true
          break
        }
      }
