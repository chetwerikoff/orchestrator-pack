...
interface StageFinalizeCliOptions extends JournalTailCliOptions {
  command: 'start-cycle' | 'publish-stage' | 'retry-pending' | 'reconcile-stage' | 'bind-published-comment' | 'produce-author-dispositions' | 'produce-artifacts' | 'check-artifacts';
  repo: string;
  issueNumber: number;
...
export function parseStageFinalizeArgs(argv: string[]): StageFinalizeCliOptions {
  ...
  if (command !== 'start-cycle' && command !== 'publish-stage' && command !== 'retry-pending' && command !== 'reconcile-stage' && command !== 'bind-published-comment' && command !== 'produce-author-dispositions' && command !== 'produce-artifacts' && command !== 'check-artifacts') {
  ...
  const reviewDirCommand = artifactCommand || command === 'produce-author-dispositions';
