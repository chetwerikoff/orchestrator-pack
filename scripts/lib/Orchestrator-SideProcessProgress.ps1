...
function Get-OrchestratorSideProcessPendingTimeoutMessage {
    ...
    # Terminal outcomes are the hot path. Avoid starting Node unless the shared
    # runtime has actually marked a timeout for this process generation.
    $path = Join-Path $dir "$ChildId.progress.json"
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { return '' }
    try {
        $progress = Get-Content -LiteralPath $path -Raw | ConvertFrom-Json
        if (-not (Test-OrchestratorSideProcessProgressProperty -Record $progress -Name 'boundedExternalCallPending') -or
            -not [bool]$progress.boundedExternalCallPending) {
            return ''
        }
        ...
        if ($progressPid -gt 0 -and $progressPid -ne $PID) {
            return ''
        }
    }
    catch {
...
function Write-OrchestratorSideProcessProgress {
    ...
    if ($existingPid -eq $PID) {
        # Terminal success is still forward-work evidence. Preserve only the
        # schema/tick/work identity so the completed tick remains attributable,
        # while deliberately clearing any prior timeout/error diagnostic state.
        foreach ($key in @(
