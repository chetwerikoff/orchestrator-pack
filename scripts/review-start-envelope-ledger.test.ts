...
import { applyLedgerReset, applyLedgerTerminal, emptyEnvelopeLedger, evaluateLedgerEscalation, isCountedTerminal, ledgerKeyForPrHead } from '../docs/review-start-envelope-ledger.mjs';
...
function terminalizeCountedFailure(dir: string, surface: string, outcome: string, extra: Record<string, unknown> = {}, prNumber = 516) {
    const script = `
...
describe('review-start-envelope-ledger unit', () => {
    ...
    it('reeval-fresh-snapshot-scopes-checks-to-planned-pr', () => {
        const src = readFileSync(snapshotHelperPath, 'utf8');
        expect(src).toMatch(/Get-ReconcileChecksByPr[\s\S]*Where-Object\s*\{\s*\[int\]\$_.number\s*-eq\s*\$prNumber\s*\}/);
    });
    ...
    it('consecutive-failure-notify-at-three', () => {
        ...
        expect(evaluateLedgerEscalation({ ledger: third.ledger, prNumber: 510, headSha: fullSha }).shouldNotify).toBe(true);
        ...
        expect(evaluateLedgerEscalation({ ledger: reset.ledger, prNumber: 510, headSha: fullSha }).consecutiveFailureCount).toBe(0);
    });
});
