...
Direct native TypeScript bins and supervised-child entrypoints must begin with a side-effect import of `scripts/toolchain/native-entrypoint-preflight.ts` before business modules. Type-only imports and other forms erased by Node type stripping do not satisfy the runtime policy. CI workflows must place exactly one literal `node-version: '22'` or `node-version: '22.x'` inside the `with` mapping of every live `actions/setup-node` step; lookalike keys under `env`, dynamic expressions, `node-version-file`, missing or duplicate selectors, and comment-only matches fail closed.
