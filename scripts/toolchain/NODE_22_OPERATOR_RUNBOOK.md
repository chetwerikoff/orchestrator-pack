...
Direct native TypeScript bins and supervised-child entrypoints must begin with a side-effect import of `scripts/toolchain/native-entrypoint-preflight.ts` before business modules. Type-only imports and other forms erased by Node type stripping do not satisfy the runtime policy. Root npm scripts that execute TypeScript must begin with the canonical Node-major preflight and chain the target only after a successful check; mentioning the check after the target does not satisfy the contract. CI workflows must place exactly one literal `node-version: '22'` or `node-version: '22.x'` inside the `with` mapping of every live `actions/setup-node` step; lookalike keys under `env`, dynamic expressions, `node-version-file`, missing or duplicate selectors, and comment-only matches fail closed.
