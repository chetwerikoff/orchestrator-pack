...
function diagnosticPath(profileKey: string, identity: string): string {
  if (!validDiagnosticIdentity(identity)) throw new Error('driver_diagnostic_identity_invalid');
  return join(profileDirs(profileKey).diagnostics, `${identity}.json`);
}
