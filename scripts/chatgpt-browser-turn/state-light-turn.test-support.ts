...
describe('Issue #1283 recovery refusal and stop ordering', () => {
  it('does not classify thrown health probes as definite loss', () => {
    expect(__testBrowserOrPageDefinitelyLost(
      { isClosed: vi.fn(() => { throw new Error('probe failed'); }) },
      { isConnected: vi.fn(() => { throw new Error('probe failed'); }) },
    )).toBe(false);
  });

  it('refuses exact-target close when Stop cannot be confirmed', async () => {
    const order: string[] = [];
    const stopControl = scalarLocator({
      count: vi.fn(async () => 1),
      click: vi.fn(async () => {
      ...
      waitFor: vi.fn(async () => {
        throw new Error('stop remained visible');
      }),
    });
    const page = {
      isClosed: vi.fn(() => false),
    ...
    const result = await __testFinalizeTurn({
      ...
      result: {
        ...
        invocation_id: 'issue-1283-stop-unconfirmed',
        configured_profile_key: 'profile-key',
        send_count: 1,
        poll_count: 3,
        goto_count: 1,
        new_chat_click_count: 0,
        navigation_count: 1,
        incidents: ['helper_failure_after_send'],
      },
    });

    expect(order).toEqual(['stop']);
    expect(page.close).not.toHaveBeenCalled();
    expect(result.incidents).toContain('owned_generation_stop_unconfirmed');
    expect(result.cleanup).toBe('skipped');
  });

  it('confirms Stop before executing an already-authorized close', async () => {
    const order: string[] = [];
    let stopVisible = true;
    const stopControl = scalarLocator({
      count: vi.fn(async () => stopVisible ? 1 : 0),
    ...
    const result = await __testFinalizeTurn({
      ...
      result: {
        ...
        scope: 'invocation',
        cause: 'observation_exhausted_no_resend',
        invocation_id: 'issue-1283-stop-before-close',
        configured_profile_key: 'profile-key',
        send_count: 1,
        poll_count: 3,
        goto_count: 1,
