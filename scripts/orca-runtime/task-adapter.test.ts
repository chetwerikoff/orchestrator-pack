...
describe('Orca spawn exact fallback', () => {
  it('uses terminal show when create omits generation and the workspace list misses the handle', () => {
    const handle = 'exact-fallback-terminal';
    const generation = 'exact-fallback-generation';
    const calls: string[][] = [];
    const runJson = vi.fn((args: readonly string[]): OrcaJsonResponse => {
      calls.push([...args]);
      const operation = `${args[0] ?? ''} ${args[1] ?? ''}`;
      if (operation === 'terminal create') {
        return { ok: true, result: { terminal: { handle, title: 'owned' } } };
      }
    ...
    expect(spawned).toMatchObject({
      status: 'ok',
      value: { identity: { runtime: 'orca', id: handle, generation } },
    });
