...
describe('distinct head counting matrix', () => {
  ...
  it('(f) two distinct terminal heads on T2 exhausts budget', () => {
    const heads = ['h1', 'h2'].map((h) => h.padEnd(40, 'f'));
  ...
  it('(g) one distinct terminal head on T1 exhausts budget', () => {
    const heads = ['a1'].map((h) => h.padEnd(40, '1'));
    const runs = heads.map((sha, idx) => ({
      prNumber: pr,
      targetSha: sha,
      status: 'changes_requested',
    ...
    const gate = run(pr, heads[0]!, runs, { issueBody: '```complexity-tier\ntier: T1\n```' });
  ...
  it('(h) four distinct terminal heads on T3 exhausts budget', () => {
    ...
    const gate = run(pr, heads[3]!, runs, { issueBody: '```complexity-tier\ntier: T3\n```' });
    expect(gate.prState?.cap).toBe(4);
    expect(gate.prState?.terminal).toBe(TERMINAL_AT_CAP_OPEN_FINDINGS);
  });
});
...
describe('at cap open findings terminal', () => {
  ...
  it('emits brief-B consumable record and suppresses every automated start surface', () => {
    const heads = ['c1', 'c2'].map((h) => h.padEnd(40, 'a'));
    const current = heads[1]!;
    const runs = heads.map((sha, idx) => ({
      prNumber: pr,
      targetSha: sha,
...
describe('review cycle cap scenario matrix', () => {
  ...
  it('at-cap head advance without clearance keeps terminal and suppresses starts', () => {
    ...
    const capped = run(pr, heads[1]!, runs, { issueBody: '```complexity-tier\ntier: T2\n```' });
    ...
    expect(afterPush.prState?.distinctHeadsReviewed).toHaveLength(2);
  });
});


  it('keeps persisted cap for in-flight cycles opened under prior mapping (Issue #1063)', () => {
    const head = 'legacy'.padEnd(40, 'l');
    const legacyCapState = {
      '646': {
        schemaVersion: 1,
        tier: 'T3',
        cap: 8,
    ...
    const gate = run(646, head, [], { capState: legacyCapState, issueBody: '```complexity-tier\ntier: T3\n```' });
    expect(gate.prState?.cap).toBe(8);
    expect(gate.prState?.tier).toBe('T3');
  });
