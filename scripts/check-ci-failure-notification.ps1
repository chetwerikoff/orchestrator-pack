...
foreach ($phrase in @('CI FAILURE DISCIPLINE', 'ci-failure-notification.ps1', 'reaction.action_succeeded', 'reactionKey=ci-failed', 'episode key')) {
  if ($example -notlike "*$phrase*") { throw "agent-orchestrator.yaml.example missing $phrase" }
}
Write-Host '[PASS] CI failure notification predicate and orchestrator rule reference (Issue #283)'
