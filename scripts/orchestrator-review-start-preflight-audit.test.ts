...
const claimedRunLib = join(repoRoot, 'scripts/lib/Invoke-OrchestratorClaimedReviewRun.ps1').replace(/'/g, "''");
...
function listAuditWriterCallSites(functionName: string): Array<{ file: string; lineNo: number; snippet: string }> {
  const matches: Array<{ file: string; lineNo: number; snippet: string }> = [];
...
$result = Invoke-OrchestratorClaimedReviewRun -SessionId 'opk-588' -ReviewCommand 'echo review' -PrNumber 588 -EventHeadSha 'DEADBEEF' -Project 'orchestrator-pack' -RepoRoot '${repoRoot.replace(/'/g, "''")}' -DryRun -AuditRoot '${auditRoot.replace(/'/g, "''")}' -LogWriter { param($m) }
$files = @(Get-ChildItem -LiteralPath (Join-Path '${auditRoot.replace(/'/g, "''")}' 'preflight') -File -ErrorAction SilentlyContinue)
@{ started = $result.started; deniedBeforeClaim = [bool]$result.deniedBeforeClaim; records = @($files | ForEach-Object { Get-Content -LiteralPath $_.FullName -Raw | ConvertFrom-Json }) } | ConvertTo-Json -Depth 6 -Compress
`;
      const result = spawnSync('pwsh', ['-NoProfile', '-Command', script], {
        cwd: repoRoot,
        encoding: 'utf8',
      ...
      expect(payload.deniedBeforeClaim).toBe(true);
      expect(payload.records.length).toBeGreaterThan(0);
      for (const record of payload.records) {
        expect(record.prNumber).toBe(588);
        expect(record.headSha).toBe('deadbeef');
        expect(record.markerState).toBe('head_unresolved');
      }
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
