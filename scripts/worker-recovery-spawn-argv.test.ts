...
import { classifySpawnAction, parseClaimPrNumberFromSpawnArgv } from '../docs/autonomous-orchestrator-boundary.mjs';
import { parseSpawnTargetFromArgv } from '../docs/spawn-worktree-grant.mjs';
import { AO_SPAWN_DISPLAY_NAME_MAX_LENGTH, validateRunnableSpawnCommand } from '../docs/ao-spawn-shape.mjs';
import { RECOVERY_SPAWN_DISPLAY_NAME_MAX_LENGTH, buildRecoverySpawnArgv, buildRecoverySpawnPrompt, classifyRecoverySpawnExit, deriveRecoverySpawnDisplayName, resolveRecoverySpawnProjectId, type BuildRecoverySpawnArgvResult, type DeriveRecoverySpawnDisplayNameResult } from './lib/worker-recovery-spawn-argv.mjs';
import { repoRoot, runPwsh } from './_test-pwsh-helpers.js';
...
describe('worker recovery AO 0.10.2 spawn argv (#638, #652)', () => {
    ...
    it('resolves --project from dead session worktree record before fallback', () => {
        expect(resolveRecoverySpawnProjectId({
    ...
    `;
        const parsed = JSON.parse(runPwsh(script)) as {
            ok: boolean;
            projectId: string;
        };
