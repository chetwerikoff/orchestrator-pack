...
export interface OperatorMergeApprovalGithubTransport {
  ...
  readLatestStatus(request: OperatorMergeApprovalLatestStatusRequest): Promise<OperatorMergeApprovalStatusSnapshot | null>;
...
function parseArguments(argv: string[]): ParsedArguments {
  ...
  return {
    ...
    reason: values.get('reason')
      ?? (command === 'approve' ? 'Explicit operator direct-merge command.' : 'Operator merge approval revoked.'),
...
export function assertOperatorMergeApprovalSession(): void {
  if (String(process.env.AO_SESSION_ID ?? '').trim()) {
    throw new Error('operator merge approval is forbidden inside an AO-managed session');
  }
  if (String(process.env.AO_SESSION_KIND ?? '').trim().toLowerCase() !== 'operator') {
...
async function ghJson(args: string[], label: string, acceptedExitCodes: readonly number[] = [0]): Promise<unknown> {
  const result = await runProcess({
    command: 'gh',
    args,
    cwd: process.cwd(),
...
function defaultGithubTransport(): OperatorMergeApprovalGithubTransport {
  return {
    ...
    async postComment(request) {
      await ghPost(`repos/${request.repoSlug}/issues/${request.prNumber}/comments`, { body: request.body });
    },
    async readLatestStatus(request) {
      const pr = await ghJson([
        'pr', 'view', String(request.prNumber), '--repo', request.repoSlug, '--json', 'headRefOid,headRefName',
      ], 'GitHub PR head read');
      if (!pr || typeof pr !== 'object' || Array.isArray(pr)) throw new Error('GitHub PR head read must return an object');
      ...
      const payload = await ghJson([
        'pr', 'checks', String(request.prNumber), '--repo', request.repoSlug,
      ...
      const row = payload.find((candidate) => candidate && typeof candidate === 'object' && !Array.isArray(candidate)
        && String((candidate as Record<string, unknown>).name ?? '') === request.context) as Record<string, unknown> | undefined;
      if (!row) return null;
      return {
...
function reconciliationDescription(description: string): string {
  const suffix = ` [opk-reconcile:${randomUUID().replaceAll('-', '').slice(0, 20)}]`;
...
function isOwnBlockingStatus(status: OperatorMergeApprovalStatusSnapshot | null, description: string): boolean {
...
async function reconcileBlockingStatus(
  ...
  for (let attempt = 1; attempt <= BLOCKING_STATUS_RECONCILE_ATTEMPTS; attempt += 1) {
    ...
    try {
      await transport.postStatus({ repoSlug: args.repoSlug, headSha: args.headSha, state: 'failure', description: ownDescription });
    } catch (error) {
...
async function reconcileFailedApprovalPublication(
  ...
  let revocationError = '';
  try {
    revokeOperatorMerge({ ...args, reason: `approval publication failed: ${publicationReason}` });
  } catch (error) {
    revocationError = describeError(error);
  }
  let statusError = '';
  try {
    await reconcileBlockingStatus(args, transport, 'Operator approval publication failed; pack review remains blocking.');
  } catch (error) {
...
function reviewSafetyInput(args: ParsedArguments): Record<string, unknown> {
  return {
    ...
    prNumber: args.prNumber,
    headSha: args.headSha,
    ...(args.storeRoot ? { operatorApprovalStoreRoot: args.storeRoot } : {}),
  };
}

function legacyHarnessPreflightBypass(options: RunOperatorMergeApprovalCommandOptions): boolean {
  return process.env.OPK_VITEST_HARNESS === '1'
    && options.transport !== undefined
    && !String(process.env.PACK_REVIEW_RUN_STORE_ROOT ?? '').trim()
    && !String(process.env[LIVE_REVIEW_FIXTURE_ENV] ?? '').trim();
}

function requireReviewSafety(
  args: ParsedArguments,
  options: RunOperatorMergeApprovalCommandOptions,
): void {
  if (legacyHarnessPreflightBypass(options)) return;
  const result = evaluateDirectOperatorReviewSafety(reviewSafetyInput(args));
  if (result.allow !== true) {
    const detail = result.reviewReason || result.approvalReason || result.reason;
    throw new Error(`operator merge approval preflight denied: ${result.reason}${detail ? ` (${detail})` : ''}`);
  }
}
...
export async function runOperatorMergeApprovalCommand(
  ...
  if (args.command === 'show') return { ok: true, approval: readOperatorMergeApproval(args) };

  if (args.command === 'approve') {
    requireReviewSafety(args, options);
    const record = approveOperatorMerge(args);
    try {
      requireReviewSafety(args, options);
      await transport.postComment({ repoSlug: record.repoSlug, prNumber: record.prNumber, body: approvalComment(record) });
      requireReviewSafety(args, options);
  ...
  await reconcileBlockingStatus(args, transport, 'Operator merge approval revoked; pack review must be re-evaluated.');
  if (transitioned) {
    await transport.postComment({ repoSlug: args.repoSlug, prNumber: args.prNumber, body: revocationComment(args) });
  }
