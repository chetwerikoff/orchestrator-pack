#!/usr/bin/env node
/**
 * Independent producer for Issue #439 AC#2 contract-evidence proof.
 */
...
const scenario = JSON.parse(
  readFileSync(
    path.join(repoRoot, 'scripts/fixtures/ci-failure-notification/live-worker-stale-same-head-fixing-ci.json'),
    'utf8',
  ),
);
...
const payload = {
  'ci-failure-progress-stale': {
    auditReason: decision.audit?.reason ?? decision.reason,
  },
};
