...
import { classifyScopedPaths } from '../../scope-guard/lib/check.js';
import {
...
const baseConstraints = {
  denylist: ['vendor/**', 'packages/core/**', '.orchestrator-pack/**'],
...
const predicateVerdictMatrix = [
  ...
  },
  {
    ...
    path: 'plugins/scope-guard/tests/secret-access.test.ts',
    matches: true,
    within: true,
    overlap: true,
    admission: false,
...
describe('validateDeclaredScope', () => {
  it('accepts declared paths within allowed_roots and outside denylist', () => {
    const result = validateDeclaredScope(
      {
        declared_paths: ['plugins/task-declaration/lib/validate.ts'],
        declared_globs: ['plugins/task-declaration/tests/**'],
      },
      baseConstraints,
    );
  ...
  it('treats an unmarked allowed-root literal as exact authority', () => {
    const result = validateDeclaredScope(
      ...
      {
        denylist: ['vendor/**'],
        allowed_roots: ['plugins/task-declaration'],
      },
    );
