...
optional `pack_ref`, else `job.workflow_sha` (commit of this reusable workflow
file in orchestrator-pack), else `main`. Do not use `github.workflow_ref` in
`workflow_call` runs — that context belongs to the caller workflow, not this
repo. The reviewer runs via `./node_modules/.bin/tsx` inside the pack checkout
so caller repos do not need `tsx` installed.
