...
function ledgerDir(repoRoot: string): string {
  return join(repoRoot, '.orchestrator-pack', 'ledger');
}
...
export function resolveChainId(options: ResolveChainIdOptions): {
  ...
  if (aoChain) {
    return { chain_id: aoChain, chain_id_source: 'runtime' };
  }
...
export function resolveParentSession(options: {
  ...
  if (fromSession) {
    return normalizeParentSession(fromSession, options.explicitSource ?? 'runtime');
  }
  if (options.envParentId?.trim()) {
    return normalizeParentSession(
      options.envParentId,
      options.explicitSource ?? 'runtime',
    );
  }
...
export function prepareLedgerRow(
  ...
  return {
    ...
    iteration_id: partial.iteration_id ?? null,
    session_id:
      partial.session_id ??
