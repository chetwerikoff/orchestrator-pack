...
export function resolveEffectiveBudgetDecision(
  ...
  const raw = env.OPK_CODEX_REVIEW_EFFECTIVE_BUDGET_MS;
  ...
  if (effectiveBudgetMs > MAX_EFFECTIVE_BUDGET_MS) {
    throw new ReviewerBudgetError(
      `OPK_CODEX_REVIEW_EFFECTIVE_BUDGET_MS exceeds ${MAX_EFFECTIVE_BUDGET_MS}`,
    );
  }
  return {
    effectiveBudgetMs,
...
export function resolveSoftDeadlineMs(
  ...
  const explicit = env.OPK_CODEX_REVIEW_SOFT_DEADLINE_MS?.trim();
  if (explicit) {
...
export function resolveTestBudgetMs(
  ...
  const explicit = env.OPK_CODEX_REVIEW_TEST_BUDGET_MS?.trim();
  if (explicit) {
    return parsePositiveIntLegacy(
      explicit,
      Math.min(
...
export function resolveTimeoutRetryMax(env: NodeJS.ProcessEnv = process.env): number {
  return parseNonNegativeIntLegacy(
    env.OPK_CODEX_REVIEW_TIMEOUT_RETRY_MAX,
...
export function buildReviewerBudgetSpawnEnv(
  ...
  return {
    ...
    OPK_REVIEW_SOFT_DEADLINE_MS: String(ledger.softDeadlineMs),
    OPK_REVIEW_TEST_BUDGET_MS: String(ledger.testBudgetMs),
    OPK_REVIEW_HARD_DEADLINE_MS: String(hardDeadlineMs),
    OPK_REVIEW_BUDGET_STARTED_MS: String(ledger.startedAtMs),
    OPK_REVIEW_RUNNER_TIMEOUT_REQUIRED_MS: String(ledger.runnerTimeoutRequiredMs),
    OPK_REVIEW_RUNNER_TIMEOUT_SECONDS: String(ledger.runnerTimeoutSeconds),
    OPK_REVIEW_RUNNER_TIMEOUT_MS: String(ledger.runnerTimeoutMs),
  };
}
