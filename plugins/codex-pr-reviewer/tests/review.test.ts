...
describe('review-mode JSONL verdict', () => {
  ...
  it('maps op-rev-26-class native hydrated findings to pack findings', () => {
    ...
    if (parsed.kind === 'findings') {
      ...
      expect(parsed.findings[0]!.path).toBe(
        'plugins/codex-pr-reviewer/tests/fixtures/review-events.jsonl',
      );
        ...
        findings: [
          {
            ...
            priority: 2,
            code_location: {
              absolute_file_path: 'plugins/codex-pr-reviewer/lib/review_jsonl.ts',
            },
          },
        ],
  ...
  it('maps absolute code_location paths to repo-relative paths', () => {
    const absolutePath = join(REPO_ROOT, 'plugins', 'codex-pr-reviewer', 'lib', 'run_review.ts');
    const parsed = parseCodexReviewOutput(
      {
        findings: [
          {
    ...
    expect(parsed.kind).toBe('findings');
    if (parsed.kind === 'findings') {
      expect(parsed.findings[0]!.path).toBe('plugins/codex-pr-reviewer/lib/run_review.ts');
    }
  });
  ...
  it('toRepoRelativePath resolves in-repo relative paths against repo root', () => {
    expect(toRepoRelativePath('plugins/codex-pr-reviewer/lib/review_jsonl.ts', REPO_ROOT)).toBe(
        ...
        findings: [
          {
            ...
            code_location: {
              absolute_file_path: 'plugins/scope-guard/lib/check.ts',
              line_range: { start: 1, end: 2 },
            },
          },
        ],
...
describe('executeReview JSONL round-trip', () => {
  ...
  it('emits repo-relative paths in AO payload for absolute JSONL code_location', () => {
    ...
    const absolutePath = join(repoRoot, 'plugins', 'codex-pr-reviewer', 'lib', 'review_jsonl.ts');
    const expectedRelative = 'plugins/codex-pr-reviewer/lib/review_jsonl.ts';
    const sessionJsonl = [
      JSON.stringify({
        type: 'event_msg',
        payload: {
...
describe('executeReview NO_FINDINGS round-trip', () => {
  ...
  it('fails on legacy prose fixture', () => {
    ...
    expect(result.reviewStdout).toBe('');
...
describe('toRuntimeFindings', () => {
  it('maps blocking scope violations to error severity', () => {
    const [finding] = toRuntimeFindings([
      {
        type: 'scope-violation',
        code: 'scope-violation:path-outside-declaration',
        severity: 'blocking',
  ...
  it('emits AO-parseable JSON payload', () => {
    const payload = parseTerminalVerdictPayload(
      emitPackReviewPayload(
        toRuntimeFindings([scopeUnavailableWarningFinding('codex-local')]),
      ),
    );
